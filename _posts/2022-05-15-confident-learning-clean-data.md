...
There are several factors at play in this process that leads to error in the dataset. The question is what can we do about it? In this post, I will touch on some of the reasons why labelling error occurs, why the errors in labels are imperative, and what are the tools and technique one can use to manage errors in the label datasets. Then, explore in detail what is [confident learning][confident_learning]. I will also cover a hands-on approach using [cleanlab], a [confident learning][confident_learning] implementation [1], to practically find noise in a multi-label dataset. 
